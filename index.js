...
(function() {
  ...
  function switchScene(scene) {
	...
	//updateEmHotspot(scene);
  }
  
  function updateEmHotspot(scene){
    //em hotspot
	var target = scene.data.id;
	if(target == '0-main-gate'){
	  var isSpot = false;
	  // Get the hotspot container for scene.
	  var container = scene.scene.hotspotContainer();
	  var hotspots = container._hotspots;
	  for(var i = 0; i < hotspots.length; i++){
		  var currentHotspot = hotspots[i]._domElement.id;
		  if(currentHotspot == "iframespot"){
			  isSpot = true;
		  }
	  }
	  if(!isSpot){
		  document.getElementById("gate").innerHTML='<div id="iframespot"><div class="message">KUEHNE+NAGEL</br>Warehouse</div></div>';
		  // Create hotspot with different sources.
		  container.createHotspot(document.getElementById('iframespot'), { yaw: 2.335, pitch: -0.0 },
			{ perspective: { radius: 3000, extraRotations: "rotateX(15deg)" }});
	  }
	}
  }
  ...
  function createLinkHotspotElement(hotspot) {
    ...
    wrapper.addEventListener('click', function() {
	  ...
	  var currentScene = findSceneById(target);
